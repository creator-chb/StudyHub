...
router.get('/', auth_js_1.authenticate, async (req, res) => {
    try {
        ...
        // 验证查询参数
        const validatedQuery = validation_js_1.taskFilterSchema.parse(req.query);
        const filter = {};
...
router.post('/', auth_js_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.userId;
        // 验证请求数据
        const validatedData = validation_js_1.createTaskSchema.parse(req.body);
        // 创建任务
...
router.put('/:id', auth_js_1.authenticate, async (req, res) => {
    try {
        ...
        const { id } = req.params;
        // 验证请求数据
        const validatedData = validation_js_1.updateTaskSchema.parse(req.body);
        // 检查任务是否存在
 ...
 * 切换任务完成状态
 */
router.patch('/:id/complete', auth_js_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        // 检查任务是否存在
...
router.post('/batch-delete', auth_js_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.userId;
        // 验证请求数据
        const validatedData = validation_js_1.batchDeleteTasksSchema.parse(req.body);
        // 批量删除
 ...
 * 批量完成任务
 */
router.post('/batch-complete', auth_js_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.userId;
        // 验证请求数据
        const validatedData = validation_js_1.batchCompleteTasksSchema.parse(req.body);
        // 批量完成
 ...
 */
router.post('/:id/open-links', auth_js_1.authenticate, async (req, res) => {
    try {
        const userId = req.user.userId;
        const { id } = req.params;
        const task = await TaskModel.findById(id, userId);
