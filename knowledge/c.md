...
function openMultipleUrls(urls) {
  ...
  // 同步打开前 5 个
  urls.slice(0, MAX_SYNC).forEach(url => {
    window.open(url, `_blank_${Date.now()}_${Math.random()}`);
  });
  
  // 超出部分提示
  if (urls.length > MAX_SYNC) {
    Toast.info(`已打开前${MAX_SYNC}个，其余请在地址栏允许弹窗后重试`);
  }
}
```
